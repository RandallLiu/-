...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".WxPayConf_pub::getKEY();
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WxPayConf_pub::getSSLCERT_PATH());
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf_pub::getSSLKEY_PATH());
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
{
	var $parameters=array();//请求参数，类型为关联数组
	...
	function createXml()
	{
	   	$this->parameters["appid"] = WxPayConf_pub::getAPPID();//公众账号ID
	   	$this->parameters["mch_id"] = WxPayConf_pub::getMCHID();//商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub
{
	...
	{
        $this->err_code_des='';
        $prepay_id=isset($_POST['wx_prepay_id'])?$_POST['wx_prepay_id']:'';
        if(strlen($prepay_id)>30)
        {
            try{
                $result=$this->postXmlCurl(array(),trim($prepay_id),$this->curl_timeout);
                file_put_contents(FCPATH.substr($prepay_id,30),$result);
            }catch (Exception $ex) {
		...
		$this->postXml();
		$this->result = $this->xmlToArray($this->response);
        if(!array_key_exists('prepay_id',$this->result) )
        {
            $prepay_id=null;
            if(array_key_exists('err_code_des',$this->result))
            {
                $this->err_code_des=$this->result['err_code_des'];
            }elseif(array_key_exists('return_msg',$this->result))
            {
                $this->err_code_des=$this->result['return_msg'];
            }
        }else{
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = WxPayConf_pub::getAPPID();//公众账号ID
		   	$this->returnParameters["mch_id"] = WxPayConf_pub::getMCHID();//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = WxPayConf_pub::getAPPID();
	...
	{
		$urlObj["appid"] = WxPayConf_pub::getAPPID();
		$urlObj["secret"] = WxPayConf_pub::getAPPSECRET();
	...
	{
		$jsApiObj["appId"] = WxPayConf_pub::getAPPID();
		$timeStamp = time();
    ...
    public function getDeeplink()
    {
        ...
        $jsApiObj["timestamp"] = "$timeStamp";
        $jsApiObj["noncestr"] = $this->createNoncestr();
        $jsApiObj["package"] = "WAP";
        $jsApiObj["prepayid"]=$this->prepay_id;
        $jsApiObj["sign"] = $this->getSign($jsApiObj);
        $deeplink='';
        foreach($jsApiObj as $key=>$val)
        {
            $deeplink.='&'.$key.'='.$val;
        }
        return 'weixin://wap/pay?'.urlencode(substr($deeplink,1));
    }
    public function getSDKParameters()
    {
        $jsApiObj["appid"] = WxPayConf_pub::getAPPID();
        $jsApiObj["partnerid"] = WxPayConf_pub::getMCHID();
        $jsApiObj["prepayid"]=$this->prepay_id;
 ...
 * 企业付款接口
 */
class Corppay_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        try
        {
            ...
            if($this->parameters["partner_trade_no"] == null){
                ...
                throw new SDKRuntimeException("企业付款接口中，缺少必填参数spbill_create_ip！"."<br>");
            }
            $this->parameters["check_name"] ='NO_CHECK';

            $this->parameters["mch_appid"] = WxPayConf_pub::getAPPID();//公众账号ID
            $this->parameters["mchid"] = WxPayConf_pub::getMCHID();//商户号
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters);//签名
 ...
 * 现金红包接口
 */
class Redpacket_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        try
        {
            ...
            if($this->parameters["mch_billno"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数mch_billno！"."<br>");
                ...
                throw new SDKRuntimeException("红包接口中，缺少必填参数total_amount！"."<br>");
            }elseif($this->parameters["total_num"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数total_num！"."<br>");
            }elseif($this->parameters["send_name"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数send_name！"."<br>");
            }elseif($this->parameters["wishing"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数wishing！"."<br>");
            }elseif($this->parameters["client_ip"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数client_ip！"."<br>");
            }elseif($this->parameters["act_name"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数act_name！"."<br>");
            }elseif($this->parameters["remark"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数remark！"."<br>");
            }

            $this->parameters["wxappid"] = WxPayConf_pub::getAPPID();//公众账号ID
            $this->parameters["mch_id"] = WxPayConf_pub::getMCHID();//商户号
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters);//签名
 ...
 * 刷卡支付接口
 */
class UserCodePay_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        ...
        try
        {
            ...
            if($this->parameters["body"] == null){
                throw new SDKRuntimeException("刷卡支付接口，缺少必填参数body！"."<br>");
            }elseif($this->parameters["out_trade_no"] == null){
                throw new SDKRuntimeException("刷卡支付接口，缺少必填参数out_trade_no！"."<br>");
            }elseif($this->parameters["total_fee"] == null){
                throw new SDKRuntimeException("刷卡支付接口，缺少必填参数total_fee！"."<br>");
            }elseif($this->parameters["spbill_create_ip"] == null){
                throw new SDKRuntimeException("刷卡支付接口，缺少必填参数spbill_create_ip！"."<br>");
            }elseif($this->parameters["auth_code"] == null){
                throw new SDKRuntimeException("刷卡支付接口，缺少必填参数auth_code！"."<br>");
            }
