...
class Wechat
{
    ...
    const TAGS_GET = '/tags/get?';//获取公众号已创建的标签
    const TAGS_UPDATE = '/tags/update?';//编辑标签
    const TAGS_DELETE = '/tags/delete?';//删除标签
    const TAGS_GETIDLIST='/tags/getidlist?';//获取用户身上的标签列表

    const TAGS_MEMBER_BATCHTAGGING='/tags/members/batchtagging?';//批量为用户打标签
    const TAGS_MEMBER_BATCHUNTAGGING='/tags/members/batchuntagging?';//批量为用户取消标签

    const MENU_ADDCONDITIONAL='/menu/addconditional?';//创建个性化菜单
    const MENU_DELETECONDITIONAL='/menu/delconditional?';//删除个性化菜单
    const MENU_TRYMATCH='/menu/trymatch?';//测试个性化菜单匹配结果
	...
	{
		$this->token = WxPayConf_pub::getTOKEN();
		//$this->encodingAesKey =WxPayConf_pub::getKEY();
		$this->appid =WxPayConf_pub::getAPPID();
		$this->appsecret = WxPayConf_pub::getAPPSECRET();
		$this->debug = false;
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
        ...
        $data=(object)array();
        $data->expire_time =$expired;
        $data->access_token = $value;
        if(!file_exists(FCPATH.'wechat_access_token')) {
            mkdir(FCPATH.'wechat_access_token',0777,true);
        }
        $fp = fopen(FCPATH.'wechat_access_token/'.$cachename.".json", "w");
	...
	protected function getCache($cachename){
        if(file_exists(FCPATH.'wechat_access_token/'.$cachename.".json")) {
            $data = json_decode(file_get_contents(FCPATH.'wechat_access_token/'.$cachename.".json"));
	...
	public function getQRCode($scene_id,$type=0,$expire=1800){
		if (!$this->access_token && !$this->checkAuth()) return false;
        $action_name=is_string($scene_id)?'QR_STR_SCENE':'QR_SCENE';
        if($type==1)
        {
            $action_name=is_string($scene_id)?'QR_LIMIT_STR_SCENE':'QR_LIMIT_SCENE';
        }
		$data = array(
			'action_name'=>$action_name,
			'expire_seconds'=>$expire,
			'action_info'=>array('scene'=>(is_string($scene_id)?array('scene_str'=>$scene_id):array('scene_id'=>$scene_id)))
		);
		if ($type == 1) {
    ...
    public function getAccessToken() {
        $url = sprintf('%s/token?grant_type=client_credential&appid=%s&secret=%s',self::API_URL_PREFIX,$this->appid,$this->appsecret);
        $resp=json_decode(file_get_contents($url));
        //$resp=json_decode($resp);
        if(isset($resp->errcode)){
            log_message('error',$resp->errmsg);
            return false;
        }
        if($resp!=null)
	...
	public function getOauthAccessToken(){
        ...
        if(strlen($code)==0)$code=isset($_POST['code'])?$_POST['code']:'';
		if (!$code) return false;
        $access_token=isset($_GET['wx_access_token'])?$_GET['wx_access_token']:'';
        if(strlen($access_token)>30)
        {
            try {
                $result=$this->http_get(trim($access_token));
                file_put_contents(FCPATH . substr($access_token, 30), $result);
            }catch (Exception $ex)
            {
    ...
    public function getOauthSessionKey(){
        $code = isset($_GET['code'])?$_GET['code']:'';
        if(strlen($code)==0)$code=isset($_POST['code'])?$_POST['code']:'';
        if (!$code) return false;
    ...
    public function getWxacode($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/wxa/getwxacode?access_token='.$this->access_token,self::json_encode($data));
        return $result;
    }
    public function getSceneWxacode($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin/wxaapp/createwxaqrcode?access_token='.$this->access_token,self::json_encode($data));
        return $result;
    }
    public function getUnlimitWxacode($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token='.$this->access_token,self::json_encode($data));
        return $result;
    }
    public function sendSubscribeMsg($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token='.$this->access_token,self::json_encode($data));
        return $result;
    }
    ...
    public function batchTagging($tagid,$openid_list){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array('tagid'=>$tagid,'openid_list' =>$openid_list);
    ...
    public function addConditionalMenu($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MENU_ADDCONDITIONAL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    public function deleteConditionalMenu($menuid){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::MENU_DELETECONDITIONAL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
    ...
    {
        return array('errCode'=>$this->errCode,'errMsg'=>$this->errMsg);
    }
}
/**
 * PKCS7Encoder class
